  ...
  UploadHandler = (function() {
    ...
    UploadHandler.prototype.upload = function(req, res) {
      ...
      form.on('file', function(field, file) {
        var upload;
        upload = _this.uploads.get(query['X-Progress-ID']);
        upload.fileName = file.name;
        if (_this.uploadDir) {
          return fs.rename(file.path, "" + _this.uploadDir + "/" + file.name);
        }
      });
      return form.addListener('progress', function(bytesReceived, bytesExpected) {
        return _this.uploads.get(query['X-Progress-ID']).updateProgress(bytesReceived, bytesExpected);
      });
    };
